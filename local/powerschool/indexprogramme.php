...
echo '<div class="d-flex flex-row align-items-center justify-content-between mx-sm-5">
            <h1>'.$getMonth. '</h1>
        <div>  
                <a href="/powereduc1/local/powerschool/indexprogramme.php?mois='.$month->previousmonth()->month.'&annee='.$month->previousmonth()->year.'&semestre='.$semestre.'&idca='.$_GET["idca"].'&idsp='.$_GET["idsp"].'&idcy='.$_GET["idcy"].'" class="btn btn-primary"> &lt;</a>
                <a href="/powereduc1/local/powerschool/indexprogramme.php?mois='.$month->nextmonth()->month.'&annee='.$month->nextmonth()->year.'&semestre='.$semestre.'&idca='.$_GET["idca"].'&idsp='.$_GET["idsp"].'&idcy='.$_GET["idcy"].'" class="btn btn-primary">&gt;</a>
        </div>
     </div>';
...
for($i = 0 ; $i < $getWeeks; $i++){
    ...
    foreach($month->days as $k => $day)
    {
      ...
      foreach($eventForDay as  $event){
        ...
        echo '<div>'
        .$heuredebut.'h -'.$heurefin.'h :   '.'<a href="/powereduc1/local/powerschool/programmeedit.php?id='.$event->id.'&idca='.$_GET["idca"].'">'.$eventday.' '.$specialitecy.'</a>
        </div>';
      }
       '</td>';
    }
